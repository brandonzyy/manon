...
**知识图谱质量全面修复** — 四项根因修复，彻底消除 phantom 节点污染和跨模块边丢失问题。全量重建验证：关系数 +74%（600→1053），跨模块边 0→51，健康评分 94→97。

- **Phantom 文件归因**（`responsible_files`）— 每个 phantom 节点现在记录负责它的源文件列表。`remove_by_file()` 在增量更新时精准清理孤立 phantom，彻底解决增量同步后图谱污染问题，无需全量重建。
- **Python 相对导入修复** — `.utils` / `..utils` 中的点号被 `posixpath.normpath` 误解析为隐藏文件名，产生双点/三点实体 ID（如 `codeindex.parsers..utils`）。修复：通过前置点号数量正确推导包层级深度。
- **项目内部绝对导入修复** — 所有非相对导入均被错误标记为外部导入，导致对项目内部类（如 `CodeGraph`、`VectorIndex`）的调用边被静默丢弃。修复：引入 `local_packages`（含 `__init__.py` 的顶层目录集合）区分真正的外部包。
- **实例方法类型推断** — Python 解析器现在追踪函数体内的 `var = ClassName()` 和 `var: ClassName = ...` 赋值模式，将 `var.method()` 调用解析为 `ClassName.method()` 并正确建立图谱边。
- **重构** — 索引管道中的内部 `_Fake*` 数据类统一替换为 `codeindex.parser` 正式类型。
- **代码健康** — `97/100`，较上版提升 3 分。
