...
与 Manon 捆绑安装，`/dao` 是一个 Claude Code skill，利用知识图谱系统化地发现并消除代码库中不必要的复杂度——不依赖猜测，基于图谱数据说话。

### 工作原理

每次执行，`/dao` 查询知识图谱获取 8 个维度的健康评分，再通过深度查询定位复杂度真正集中的位置。发现的问题按三层分类记录：

| 层级 | 原则 | 示例 |
|------|------|------|
| **架构层 (A)** | A1–A7 | 不必要的分层、过度模块化、过早泛化 |
| **模块层 (M)** | M1–M4 | 功能臃肿、边界不清、重复、过度依赖 |
| **代码层 (C)** | C1–C8 | 死代码、过度碎片化、循环依赖、低内聚、桶文件 |

### 三层执行模型

**架构和模块层 issue** 需要人工判断。`/dao` 展示交互面板列出待处理的 A/M 问题，用户选择后 Claude 进入 Plan mode 设计方案，人工审批后执行、测试、提交，并重新同步图谱。每次会话处理一个 issue。

**代码层 issue** 自动循环执行，无需确认。每个候选项在执行前通过验证门控（如 C2 合并需检查 git 历史、操作类型一致性、命名准确性；C4 删除需通过图谱确认零调用者）。每次修复后提交并同步图谱，然后继续下一个候选项。

### 安全门控

- **C2（合并）** — 仅合并从未被独立修改、操作类型一致、合并结果能用一句话描述清楚的文件。
- **C4（删除）** — 仅删除经 `manon_graph` 确认为零调用者的实体。图谱覆盖盲区（如仅被外部脚本调用的文件）跳过不处理。
- **所有层级** — 测试失败则停止会话；看起来是有意设计的耦合，不做简化。

### Issue 追踪

Issue 存储在项目根目录的 `.dao/issues.json` 中，每次操作后更新。每次提交后通过 `manon_impact` 重新同步图谱，健康评分实时反映当前状态。

```
/dao          — 单次迭代：查询图谱 → 展示面板 → 修复一个 issue → 停止
```
