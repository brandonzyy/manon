...
async def lifespan(app: FastAPI):
    ...
    # Configure shared saas_client
    from shared import saas_client
    saas_client.configure(settings.saas_url, settings.saas_api_key)
    log.info("SaaS backend: %s", settings.saas_url)
    # Start monitor broadcast loop
    monitor_task = asyncio.create_task(_monitor_broadcast_loop())
    log.info("Manon Gateway ready (saas backend mode)")
    # Auto-open browser
...
def _include_routers():
    from .routers import projects, query, settings
...
if __name__ == "__main__":
    import uvicorn
    settings = get_settings()
    uvicorn.run("web.main:app", host="0.0.0.0", port=settings.port, log_level="info")
