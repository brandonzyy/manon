"""Tests for mcp/_config.py — version, geo-routing, version check."""
import json
import pytest
from pathlib import Path
from unittest.mock import patch, MagicMock

from mcp._config import (
...
class TestGetCachedRegion:
    def test_with_cache_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mcp._config._REGION_CACHE", cache)
        assert _get_cached_region() == "INTL"

    def test_without_cache_defaults_cn(self, tmp_path, monkeypatch):
        monkeypatch.setattr("mcp._config._REGION_CACHE", tmp_path / "nope.json")
...
class TestResolveApiUrl:
    def test_explicit_url(self, monkeypatch):
        monkeypatch.setattr("mcp._config._explicit_url", "http://custom:3700")
        assert _resolve_api_url() == "http://custom:3700"

    def test_cn_region(self, monkeypatch):
        monkeypatch.setattr("mcp._config._explicit_url", "")
        monkeypatch.setattr("mcp._config._get_cached_region", lambda: "CN")
...
class TestGitBranch:
    def test_cn_branch(self, monkeypatch):
        monkeypatch.setattr("mcp._config.REGION", "CN")
        assert _git_branch() == GIT_BRANCH_CN

    def test_intl_branch(self, monkeypatch):
        monkeypatch.setattr("mcp._config.REGION", "INTL")
