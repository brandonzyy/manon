"""Tests for application.mcp_sync_service."""
...
def test_create_repo_with_local_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("application.mcp_sync_service.count_scannable_files", lambda path: 7)
    monkeypatch.setattr(
        "application.mcp_sync_service.set_project",
...
def test_delete_repo_clears_local_binding(monkeypatch):
    ...
    monkeypatch.setattr("application.mcp_sync_service.find_project_by_repo_id", lambda repo_id: ("C:/demo", {}))
    monkeypatch.setattr("application.mcp_sync_service.load_projects", lambda: {"projects": {"C:/demo": {}}})
    monkeypatch.setattr("application.mcp_sync_service.save_projects", lambda data: saved.update(data))
