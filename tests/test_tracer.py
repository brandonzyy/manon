...
class TestMergeDynamic:
    def _make_graph(self) -> CodeGraph:
        g = CodeGraph()
        g.add_entity(Entity(id="mcp._tools.register", kind="function", name="register", file_path="mcp/_tools.py"))
        g.add_entity(Entity(id="mcp._client._get", kind="function", name="_get", file_path="mcp/_client.py"))
        g.add_entity(Entity(id="shared.ast_sync.scan", kind="function", name="scan", file_path="shared/ast_sync.py"))
        return g

    def test_merge_adds_edges(self):
        g = self._make_graph()
        edges = {"mcp._tools.register->mcp._client._get": 5}
        stats = merge_dynamic_edges(g, edges)
        assert stats["added"] == 1
        assert stats["skipped"] == 0
        # Verify edge exists in graph
        assert g._g.has_edge("mcp._tools.register", "mcp._client._get")
        edata = g._g.edges["mcp._tools.register", "mcp._client._get"]
    ...
    def test_replace_removes_old_dynamic_edges(self):
        ...
        g.add_relation(Relation(
            src_id="mcp._tools.register", tgt_id="shared.ast_sync.scan",
            kind="calls", file_path=DYNAMIC_FILE_PATH, weight=1.0,
        ))
        assert g._g.has_edge("mcp._tools.register", "shared.ast_sync.scan")
        # Merge with replace=True should remove old dynamic edge
        edges = {"mcp._tools.register->mcp._client._get": 3}
        stats = merge_dynamic_edges(g, edges, replace=True)
        assert stats["removed"] == 1
        assert stats["added"] == 1
        assert not g._g.has_edge("mcp._tools.register", "shared.ast_sync.scan")
